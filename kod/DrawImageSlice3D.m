function [ fighandle, Handle_axis, surfh, I ] = DrawImageSlice3D( dicom_filename,  fighandle,  transparency )
%============================
...
%transparency = binaryImage .* transparency;
alphachannel = binaryImage.*0.2;
...
[f,g,h]=meshgrid(linspace(x(1,1), x(2,2),256),linspace(y(1,1),y(2,2),256),linspace(z(1,1),z(2,2),256));
xx(:,:)= f(:,:,1);
yy(:,:)= g(:,:,1);
if(~dinfo.ImageOrientationPatient(5))
    yy=yy';
end
zz(:,:)= h(:,1,:);
zz=zz';
% plot surface
surfh = surface('XData',xx,'YData',yy,'ZData',zz,...
'CData', I,...
'EdgeColor','none',...
'FaceAlpha','flat',...
'FaceColor','flat',...%'texturemap'
'AlphaData', alphachannel,...
'LineStyle','none',...
'Marker','none',...
'MarkerFaceColor','none',...
'MarkerEdgeColor','none',...
'AlphaDataMapping','none',...%doplnil som
'CDataMapping','direct');    
%set transparency level
%set( surfh, 'FaceAlpha', transparency ); %
%set( surfh, 'AlphaData', alphachannel );
